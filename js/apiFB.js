/*1380638874,178541853,JIT Construction: v952535,en_US*/
...
try {
	window.FB || (function (window) {
		...
		__d("sdk.createIframe", ["copyProperties", "guid", "hasNamePropertyBug", "DOMEventListener"], function (a, b, c, d, e, f) {
			...
			function k(l) {
				...
				if (s.width)
					m.width = s.width + 'px';
				if (s.height)
					m.height = s.height + 'px';
				delete s.height;
				delete s.width;
				for (var t in s)
					if (s.hasOwnProperty(t))
						m.setAttribute(t, s[t]);
				g(m.style, p);
				m.src = "javascript:false";
				o.appendChild(m);
				if (r)
					var u = j.add(m, 'load', function () {
							u.remove();
							r();
						});
				m.src = q;
		...
		__d("sdk.Event", [], function (a, b, c, d, e, f) {
			var g = {
				...
				unsubscribe : function (h, i) {
					...
					if (j)
						ES5(j, 'forEach', true, function (k, l) {
							if (k == i)
								j[l] = null;
						});
				},
		...
		__d("sdk.XD", ["sdk.Content", "sdk.createIframe", "sdk.Event", "guid", "Log", "QueryString", "Queue", "resolveURI", "resolveWindow", "sdk.RPC", "sdk.Runtime", "UrlMap", "URL", "wrapFunction", "XDM", "XDConfig"], function (a, b, c, d, e, f) {
			...
			function la(pa, qa) {
				if (qa && qa !== 'native' && !s(qa).isFacebookURL())
					return;
				if (typeof pa == 'string') {
					if (/^FB_RPC:/.test(pa)) {
				...
				if (pa.access_token)
					q.setSecure(/^https/.test(da));
				if (pa.cb) {
					var sa = oa._callbacks[pa.cb];
					if (!oa._forever[pa.cb])
						delete oa._callbacks[pa.cb];
					if (sa)
						sa(pa);
				}
			}
			...
			function na(pa, qa) {
				...
				if (ta === 'fragment')
					window.FB_XD_onMessage = t(la, 'entry', 'XD:fragment');
				fa = true;
			}
			var oa = {
				rpc : p,
		...
		__d("sdk.Auth", ["sdk.Cookie", "copyProperties", "sdk.createIframe", "DOMWrapper", "sdk.feature", "sdk.getContextType", "guid", "sdk.Impressions", "Log", "ObservableMixin", "QueryString", "sdk.Runtime", "sdk.SignedRequest", "UrlMap", "URL", "sdk.XD"], function (a, b, c, d, e, f) {
			...
			function z(fa, ga) {
				...
				if (la || na)
					y.inform('login', qa);
				if (oa)
					y.inform('authresponse.change', qa);
				if (pa)
					y.inform('status.change', qa);
				return qa;
			}
			function aa() {
				return w;
				...
				return function (ia) {
					...
					if (ia && ia.access_token) {
						...
						if (r.getUseCookie()) {
							...
							ma = g.getDomain();
							if (!ma && ia.base_domain)
								g.setDomain('.' + ia.base_domain);
			...
			function ba(da, ea, fa, ga) {
				...
				default:
					return m.guard(function () {
						...
						function ia(qa) {
							if (ES5('Array', 'isArray', false, qa))
								return ES5(qa, 'map', true, ia);
							if (qa && typeof qa === 'object' && qa.__wrapped)
								return qa.__wrapped;
						...
						if (ka && typeof ka === 'object') {
							...
							for (var oa in ka) {
								var pa = ka[oa];
								if (typeof pa !== 'function' || oa === 'constructor')
									continue;
								ma = false;
				...
				for (var j in h)
					if (h.hasOwnProperty(j)) {
						...
						if (null === k || undefined === k) {
							...
							i[j] = ES5('JSON', 'stringify', false, k);
					}
				return i;
			}
			e.exports = g;
		...
		__d("sdk.api", ["ApiClient", "sdk.Runtime"], function (a, b, c, d, e, f) {
			...
			g.subscribe('request.complete', function (k, l, m, n) {
				...
				if (n && typeof n == 'object')
					if (n.error) {
						if (n.error == 'invalid_token' || (n.error.type == 'OAuthException' && n.error.code == 190))
							o = true;
					} else if (n.error_code)
						if (n.error_code == '190')
							o = true;
				if (o && i === h.getAccessToken())
					h.setAccessToken(null);
			});
			g.subscribe('request.complete', function (k, l, m, n) {
				if (((k == '/me/permissions' && l === 'delete') || (k == '/restserver.php' && m.method == 'Auth.revokeAuthorization')) && n === true)
					h.setAccessToken(null);
			});
			function j() {
			...
			p = n.extend({
					...
					trackEvents : function (r) {
						if (!r)
							return this;
						if (typeof r === 'string')
							r = ES5('JSON', 'parse', false, r);
						for (var s in r)
							if (r.hasOwnProperty(s))
								this.trackEvent(s, r[s]);
						return this;
					}
				}, l),
			q = {
				...
				_hideIPadOverlay : function () {
					if (o.ipad())
						q._overlayEl.className = 'hidden';
				},
				showLoader : function (r, s) {
					q._showIPadOverlay();
					if (!q._loaderEl)
						q._loaderEl = q._findRoot(o.mobile() ? q._createMobileLoader() : q._createWWWLoader(s));
					if (!r)
						r = function () {};
					...
					if (u)
						u.ontouchstart = t.onclick;
					q._makeActive(q._loaderEl);
				},
				_hideLoader : function () {
					if (q._loaderEl && q._loaderEl == q._active)
						q._loaderEl.style.top = '-10000px';
				},
				...
				_centerActive : function (r) {
					...
					z = (t.height - v) / 2;
					if (r)
						z = r.scrollTop - r.offsetTop + (r.clientHeight - v) / 2;
					if (z < x) {
						z = x;
					} else if (z > y)
						z = y;
				...
				create : function (r) {
					...
					if (o.ie()) {
						...
						u += o.mobile() ? ' fb_dialog_mobile' : ' fb_dialog_advanced';
					if (r.content)
						h.append(r.content, t);
					s.className = u;
				...
				show : function (r) {
					...
					if (s) {
						...
						q._stack.push(s);
						if ('fbCallID' in r)
							q.get(r.fbCallID).inform('iframe_show').trackEvent('show');
					}
				},
				hide : function (r) {
					var s = q._findRoot(r);
					...
					if (s == q._active) {
						...
						q._hideIPadOverlay();
						if ('fbCallID' in r)
							q.get(r.fbCallID).inform('iframe_hide').trackEvent('hide');
					}
				},
				remove : function (r) {
					r = q._findRoot(r);
					if (r) {
						...
						} else if (q._active === null && q._stack.length > 0)
							q.show(q._stack.pop());
						setTimeout(function () {
			...
			da = {
				...
				checkOauthDisplay : function (ea) {
					...
					for (var ha = 0; ha < ga.length; ha++)
						if (!x.initSitevars.iframePermissions[ES5(ga[ha], 'trim', true)])
							return 'popup';
					return ea.display;
				},
		...
		__d("sdk.ui", ["Assert", "copyProperties", "sdk.feature", "sdk.Impressions", "Log", "sdk.UIServer"], function (a, b, c, d, e, f) {
			...
			function m(n, o) {
				...
				if ((p == 'permissions.request' || p == 'permissions.oauth') && (n.display == 'iframe' || n.display == 'dialog'))
					n.display = l.checkOauthDisplay(n);
				...
				if (q)
					r.dialog.subscribe('e2e:end', function (u) {
						u.method = p;
						u.display = s;
		...
		__d("sdk.Canvas.Plugin", ["sdk.api", "sdk.RPC", "Log", "sdk.Runtime", "createArrayFrom"], function (a, b, c, d, e, f) {
			...
			function s(y) {
				...
				if (aa.test(y.getAttribute('wmode')))
					return false;
				for (var ba = 0; ba < y.childNodes.length; ba++) {
					var ca = y.childNodes[ba];
					if (/param/i.test(ca.nodeName) && /wmode/i.test(ca.name) && aa.test(ca.value))
						return false;
				}
				return true;
			}
			...
			function u(y) {
				...
				ES5(z, 'forEach', true, function (aa) {
					...
					if (Math.random() <= 1 / 1000)
						g(j.getClientID() + '/occludespopups', 'post', {});
				});
			}
		...
		__d("sdk.Canvas.Prefetcher", ["sdk.api", "createArrayFrom", "sdk.Runtime", "CanvasPrefetcherConfig"], function (a, b, c, d, e, f) {
			...
			function p() {
				...
				if (n == k.AUTOMATIC)
					ES5(ES5('Object', 'keys', false, u), 'forEach', true, function (v) {
						var w = u[v];
			...
			function q() {
				if (!j.isEnvironment(j.ENVIRONMENTS.CANVAS) || !j.getClientID() || !l)
					return;
				if (Math.random() > 1 / l || m == '*' || ~ES5(m, 'indexOf', true, j.getClientID()))
					return;
				setTimeout(p, 30000);
			}
			function r(u) {
					...
					toFql : function () {
						...
						case 'in':
							if (this.where.value.length == 1) {
								...
								p += this.where.key + ' in (' + ES5(this.where.value, 'map', true, m).join(',') + ')';
							break;
						}
						return p;
					},
				...
				_process : function (n) {
					...
					g('/fql', 'GET', s, function (u) {
						if (u.error) {
							...
							ES5(u.data, 'forEach', true, function (v) {
			...
			function p(q) {
				...
				if (q.cookie) {
					...
					if (typeof q.cookie === 'string')
						g.setDomain(q.cookie);
				}
				if (q.kidDirectedSite)
		...
		__d("XFBML", ["Assert", "copyProperties", "createArrayFrom", "sdk.DOM", "sdk.feature", "sdk.Impressions", "Log", "ObservableMixin", "runOnce", "UserAgent"], function (a, b, c, d, e, f) {
			...
			function x(ca) {
				...
				if (da.length === 0)
					return undefined;
				if (ca.getAttribute('fb-xfbml-state') || !ca.childNodes || ca.childNodes.length === 0 || (ca.childNodes.length === 1 && ca.childNodes[0].nodeType === 3) || (ca.children.length === 1 && u(ca.children[0], 'className') === 'fb-xfbml-parse-ignore'))
					return r[da[0]];
			}
			function y(ca) {
				var da = {};
			...
			function aa(ca, da, ea) {
				...
				ES5(i(ca.getElementsByTagName('*')), 'forEach', true, function (ka) {
					if (!ea && ka.getAttribute('fb-xfbml-state'))
						return;
					if (ka.nodeType !== 1)
						return;
					var la = w(ka) || x(ka);
					if (!la)
						return;
					if (p.ie() < 9 && ka.scopeName)
						ka = z(ka, la.xmlns, la.localName);
					ga++;
					ha++;
				...
				setTimeout(function () {
					if (ga > 0)
						m.warn('%s tags failed to render in %s ms', ga, ja);
				}, ja);
				ia();
			}
		...
		__d("IframePlugin", ["sdk.Auth", "sdk.createIframe", "copyProperties", "sdk.DOM", "sdk.Event", "guid", "Log", "ObservableMixin", "PluginPipe", "QueryString", "resolveURI", "sdk.Runtime", "Type", "UrlMap", "UserAgent", "sdk.XD"], function (a, b, c, d, e, f) {
			...
			function x(ea, fa, ga) {
				if (fa || fa === 0)
					ea.style.width = fa + 'px';
				if (ga || ga === 0)
					ea.style.height = ga + 'px';
			}
			function y(ea) {
				return function (fa) {
					var ga = {
					...
					setupAndValidate : function () {
						...
						if (!o.href) {
							...
							if (!q) {
								...
								if (q && ES5(q, 'indexOf', true, '#') > 0)
									q = q.substring(0, ES5(q, 'indexOf', true, '#'));
							}
							if (q) {
								o.fb_comment_id = q;
			...
			h = {
				ate : function (i, j, k, l) {
					...
					q = setInterval(ES5(function () {
								if (!p)
									p = ES5('Date', 'now', false);
								var r = 1;
								if (k != 0)
									r = Math.min((ES5('Date', 'now', false) - p) / k, 1);
								for (var s in j)
									if (j.hasOwnProperty(s)) {
										...
										ES5(n[s], 'forEach', true, function (w, x) {
											if (isNaN(o[s][x].numPart) && o[s][x].textPart == '?') {
												...
												v += (w.numPart + Math.ceil((o[s][x].numPart - w.numPart) * Math.sin(Math.PI / 2 * r))) + o[s][x].textPart + ' ';
										});
										g.setStyle(i, s, v);
									}
								if (r == 1) {
			...
			x = n.extend({
					...
					_renderConnectBar : function (y) {
						...
						if (this._page.parentNode) {
							...
							ba = parseInt(m.getStyle(this._page, 'marginTop'), 10);
						ba = isNaN(ba) ? 0 : ba;
						...
						if (w.ie()) {
							...
							ca.backgroundPosition = '? ' + this._initialHeight + 'px';
			...
			m = h.extend({
					...
					oneTimeSetup : function () {
						this.subscribe('xd.sdk_event', ES5(function (n) {
								...
								} else if (n.event == 'edge.remove')
									k.invokeHandler(this.getAttribute('on-remove'), this, [this._attr.href]);
							}, 'bind', true, this));
					...
					_getWidthOffset : function (n) {
						...
						if (l.getRtl()) {
							...
							t = true;
						if (t)
							o += parseFloat(n['anchorGeometry[x]']) - s;
						return o;
					},
			...
			i = g.extend({
					...
					_createEdgeCommentWidget : function (j, k) {
						if ('send' in this._attr && 'widget_type' in j && j.widget_type == 'send') {
							...
							return this.parentCall("_createEdgeCommentWidget", j, k);
					},
					getIframeTitle : function () {
		...
		__d("sdk.XFBML.Name", ["copyProperties", "sdk.Data", "escapeHTML", "sdk.Event", "sdk.XFBML.Element", "sdk.Helper", "Log", "sdk.Runtime"], function (a, b, c, d, e, f) {
			...
			o = k.extend({
					...
					_renderOther : function (p) {
						...
						if (!q)
							q = i(this.getAttribute('if-cant-see', 'Facebook User'));
						if (q) {
							if (this._getBoolAttribute('capitalize', false))
								q = l.upperCaseFirstChar(q);
							if (p && this._linked) {
			...
			l = i.extend({
					...
					setupAndValidate : function () {
						...
						if (this._attr.onvalidate)
							this.subscribe('xd.validate', ES5(function (m) {
		...
		__d("legacy:fb.xfbml", ["Assert", "sdk.domReady", "sdk.Event", "FB", "IframePlugin", "PluginTags", "wrapFunction", "XFBML", "sdk.XFBML.Comments", "sdk.XFBML.CommentsCount", "sdk.XFBML.ConnectBar", "sdk.XFBML.Like", "sdk.XFBML.LiveStream", "sdk.XFBML.LoginButton", "sdk.XFBML.Name", "sdk.XFBML.RecommendationsBar", "sdk.XFBML.Registration", "sdk.XFBML.SocialContext"], function (a, b, c, d) {
			...
			g.subscribe('init:post', function (o) {
				if (o.xfbml)
					setTimeout(k(ES5(f, 'bind', true, null, l.parse), 'entry', 'init:post:xfbml.parse'), 0);
			});
